...
def is_us_match(x):     # United States 'US'
    try:
        ...
        valid_terms = ['US', 'USA', 'UNITED STATES', 'AMERICA']
...
def is_nz_match(x):     # New Zealand 'nz'
    ...
    try:
        x=str(x)
        tokens = ''.join(c for c in x if c.isalnum() or c.isspace()).upper().split()
        valid_terms = ['NEW ZEALAND', 'AOTEAROA', 'nZ']
...
def is_ar_match(x):     # Argentine 'AR'
    try:
        x=str(x)
        tokens = x.upper().split()
        valid_terms = ['ARGENTINA', 'BUENOS AIRES', 'CIUDAD AUTONOMA DE BUENOS AIRES']
...
def is_au_match(x):     # Australia 'AU'
    try:
        x=str(x)
        tokens = x.upper().split()
        valid_terms = ['AUSTRALIA', 'CANBERRA', 'SYDNEY']
...
def is_other_match(x):     # Not a country 'other'
    try:
        ...
        valid_terms = ['CONTRACT', 'GLOBAL', 'RAISES', 'FINANCE', 'BENEFITS']
